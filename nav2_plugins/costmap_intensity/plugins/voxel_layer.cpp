#include "costmap_intensity/voxel_layer.hpp"
...
PLUGINLIB_EXPORT_CLASS(costmap_intensity::VoxelLayerIntensity, nav2_costmap_2d::Layer)
...
void VoxelLayerIntensity::onInitialize()
{
  ObstacleLayerIntensity::onInitialize();
  ...
  dyn_params_handler_ = node->add_on_set_parameters_callback(
    std::bind(
      &VoxelLayerIntensity::dynamicParametersCallback,
      this, std::placeholders::_1));
}

VoxelLayerIntensity::~VoxelLayerIntensity()
{
  dyn_params_handler_.reset();
}

void VoxelLayerIntensity::matchSize()
{
...
void VoxelLayerIntensity::reset()
{
...
void VoxelLayerIntensity::resetMaps()
{
  // Call the base class method before adding our own functionality
  // Note: at the time this was written, ObstacleLayerIntensity doesn't implement
  ...
  ObstacleLayerIntensity::resetMaps();
  voxel_grid_.reset();
}

void VoxelLayerIntensity::updateBounds(
  ...
  double * min_y, double * max_x, double * max_y)
{
    ...
    ++it)
  {
    ...
    sensor_msgs::PointCloud2ConstIterator<uint8_t> iter_i(cloud, "intensity");

    for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z, ++iter_i) {
      // if the obstacle is too high or too far away from the robot we won't add it
      if (*iter_i > max_obstacle_intensity_) {
        continue;
      }
...
void VoxelLayerIntensity::raytraceFreespace(
...
void VoxelLayerIntensity::updateOrigin(double new_origin_x, double new_origin_y)
{
  ...
  std::vector<rclcpp::Parameter> parameters)
{
  ...
  for (auto parameter : parameters) {
    ...
    if (param_type == ParameterType::PARAMETER_DOUBLE) {
      if (param_name == name_ + "." + "max_obstacle_intensity") {
