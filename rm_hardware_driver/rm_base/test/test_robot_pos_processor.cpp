...
TEST(RobotPosProcessor, test_robot_pos_processor)
{
  ...
  packet.load_data(static_cast<uint8_t>(rm_base::RecvID::ROBOTPOS), 1);
  packet.load_data(robot_pos, 2);
  ...
  EXPECT_TRUE(
    rm_base::ProcessFactory::create(
      static_cast<uint8_t>(rm_base::RecvID::ROBOTPOS),
      node.get()));
  EXPECT_TRUE(
