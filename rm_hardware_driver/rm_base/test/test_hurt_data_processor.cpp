...
TEST(HurtDataProcessor, test_hurt_data_processor)
{
  ext_hurt_data_t hurt_data;
  hurt_data.armor_id = 0x01;
  hurt_data.hurt_type = 0x01;
  rmoss_base::FixedPacket64 packet;
  packet.load_data(static_cast<uint8_t>(rm_base::RecvID::HURTDATA), 1);
  packet.load_data(hurt_data, 2);
  packet.set_check_byte(rm_base::Get_CRC8_Check_Sum(packet.buffer() + 1, 61, rm_base::CRC8_INIT));

  rclcpp::init(0, nullptr);
  auto node = rclcpp::Node::make_shared("test_hurt_data_frame");
  EXPECT_TRUE(
    rm_base::ProcessFactory::create(
      static_cast<uint8_t>(rm_base::RecvID::HURTDATA),
      node.get()));
  EXPECT_TRUE(
